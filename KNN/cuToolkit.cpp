...
bool cuToolkit::cuLU(double * d_A, double *d_A_INV, int N)
{
	...
	cudaMalloc((void **)& Ori_d_A, N *N * sizeof(double));
	cudaMalloc((void **)& idty, N *N * sizeof(double));
	cudaDeviceSynchronize();
	cudaMemcpy(Ori_d_A, d_A, N *N * sizeof(double),cudaMemcpyDeviceToDevice);
	cudaMemcpy(idty, d_A_INV, N *N * sizeof(double), cudaMemcpyDeviceToDevice);
	cublasstatus = cudaGetLastError();
	if (cublasstatus != cudaSuccess)
	{
		throw (cudaGetErrorString(cublasstatus));
	...
	status=cusolverDnDgetrs(handle, CUBLAS_OP_N, N, N, d_A, N, dLUPivots, d_A_INV, N, dLUInfo);
	cudaDeviceSynchronize();
	cublasstatus = cudaGetLastError();
	if (cublasstatus != cudaSuccess)
	{
	...
	const double beta = -1;
	cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha, Ori_d_A, N, d_A_INV, N, &beta, idty, N);
	cudaDeviceSynchronize();
	cublasstatus = cudaGetLastError();
	if (cublasstatus != cudaSuccess)
	{
	...
	cudaDeviceSynchronize();
	double dI_nrm2 = 0.0;
	cublasDnrm2_v2(cublasH, N*N, idty, 1, &dI_nrm2);
	cudaDeviceSynchronize();
	cublasstatus = cudaGetLastError();
	if (cublasstatus != cudaSuccess)
	{
	...
	return dI_nrm2 < 1e-6 ? true : false;;
}
...
bool cuToolkit::cuSVD(double * d_A, double * d_A_INV, int N)
{
	...
	cudaStat = cudaMalloc((void **)& Ori_d_A, sizeof(double)*N*N);
	cudaStat = cudaMalloc((void **)& d_A_d_A_INV, sizeof(double)*N*N);
	cudaDeviceSynchronize();
	...
	cudaStat = cudaMemcpy(Ori_d_A, d_A, sizeof(double)*N*N, cudaMemcpyDeviceToDevice);
	cudaMemset(d_A_d_A_INV, 0, sizeof(double)*N*N);
	cudaDeviceSynchronize();
	cudaStat = cudaGetLastError();
	if (cudaStat != cudaSuccess)
	{
	...
	//check inversed
	cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha, Ori_d_A, N, d_A_INV, N, &betaZero, d_A_d_A_INV, N);
	cudaDeviceSynchronize();
	cudaStat = cudaGetLastError();
	if (cudaStat != cudaSuccess)
	{
	...
	cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha, d_A_d_A_INV, N, Ori_d_A, N, &betaMinusOne, Ori_d_A, N);
	cudaDeviceSynchronize();
	double dI_nrm2 = 0.0;
	cublas_status = cublasDnrm2_v2(
		cublasH, N*N, Ori_d_A, 1, &dI_nrm2);
	cudaDeviceSynchronize();
	assert(CUBLAS_STATUS_SUCCESS == cublas_status);
	cudaFree(Ori_d_A);
	cudaFree(d_A_d_A_INV);
	cudaFree(d_S);
	cudaFree(d_U);
	cudaFree(d_VT);
	cudaFree(devInfo);
...
bool cuToolkit::cuCholesky(double * d_A, double * d_A_INV, int N)
{
	...
	cusolverStatus = cusolverDnDpotrs(handle, uplo, N, N, d_A, N,
		d_A_INV, N, d_info);
	cudaDeviceSynchronize();
	cudaStatus = cudaGetLastError();
	if (cudaStatus != cudaSuccess)
...
void InverseTest()
{
	...
	MatrixXd a = MatrixXd::Random(nrank, nrank);
	Eigen::MatrixXd IdentityMatrix(nrank, nrank);
	...
	a.col(5) << b;
	double *h_ainv;
	h_ainv = (double *)malloc(nrank*nrank * sizeof(double));
	memset(h_ainv, 0, nrank*nrank * sizeof(double));
...
// 	std::cout << "\\\\\\\\\\\\\\\\\\\\\\\\\\\\" << std::endl;
	{
		...
		cudaDeviceSynchronize();
		clock_t t1 = clock();
		bool inverstatus=cuToolkit::cuCholesky(aorg, ainv, nrank);
		std::cout << "cuCholesky inverse Elapse Time : " << (clock() - t1) * 1.0 / CLOCKS_PER_SEC * 1000 << " ms" << std::endl;
		if (inverstatus)
		{
			double *ai;
			...
			bool a_solution_exists = (a*a2).isApprox(IdentityMatrix, 1e-10);
